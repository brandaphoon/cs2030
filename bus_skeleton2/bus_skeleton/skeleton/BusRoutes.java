...
class BusRoutes {
  ...
  public CompletableFuture<String> description() {
     ...
     * += "From " + this.stop + "\n";
     * 
     * for (BusService service: services.keySet()) { Set<BusStop> stops =
     * services.get(service); result += describeService(service, stops); } return
     * result;
     */

    String str = "Search for: " + this.stop + " <-> " + name + ":\n" + "From " + this.stop + "\n";
    CompletableFuture<String> result = CompletableFuture.supplyAsync(() -> str);
    for (BusService service : services.keySet()) {
      CompletableFuture<Set<BusStop>> stops = services.get(service);
      result = result.thenApply(x -> x).thenCombine(describeService(service, stops), (x, y) -> x + y);
    }
    return result;
  }
  ...
  public CompletableFuture<String> describeService(BusService service, CompletableFuture<Set<BusStop>> stops) {
    /*
     * if (stops.isEmpty()) { return ""; } return stops.stream() .filter(stop ->
     * stop != this.stop) .reduce("- Can take " + service + " to:\n", (str, stop) ->
     * str += "  - " + stop + "\n", (str1, str2) -> str1 + str2);
     */
    return stops.thenApply(s -> {
      if (s.isEmpty()) {
        ...
        return s.stream().filter(stop -> stop != this.stop).reduce("- Can take " + service + " to:\n",
            (str, stop) -> str += "  - " + stop + "\n", (str1, str2) -> str1 + str2);
      }
    });
