...
public class Server {
    ...
    private final static ArrayList<Customer> EMPTY_QUEUE = new ArrayList<>();
    ...
    public Server(int id, Optional<Customer> c1, ArrayList<Customer> waiting, int maxQueue, double serviceTime,
            double completionTime, boolean isResting) {
        this.id = id;
        this.cust = c1;
        this.waiting = waiting;
    ...
    public Optional<Server> wait(Customer cust) {
        if (this.canWait()) {
            ...
            newWaiting.add(cust);
            return Optional
                    .of(new Server(this.id, this.getCustomer().get(), newWaiting, this.maxQueue, this.serviceTime));
        } else {
            return Optional.empty();
        }
    }

    public Server rest(double restDuration) {

        return new Server(this.id, this.cust, this.waiting, this.maxQueue, this.serviceTime,
                this.completionTime + restDuration, true);
