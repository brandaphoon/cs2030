...
/**
 * BusService encapsulate a bus service with a String id. It supports querying
 * for the list of bus stops served by this service.
 *
...
class BusService {
  ...
  /**
   * Construct a BusService object with a given id. An empty Set of bus stops is
   * initialized.
   * 
  ...
  /**
   * Get the current list of bus stops as a set. Query the web server if bus stops
   * are not retrieved before.
   * 
  ...
  public CompletableFuture<Set<BusStop>> getBusStops() {
    ...
    CompletableFuture<Scanner> sc = BusAPI.getBusStopsServedBy(serviceId).thenApply(x -> new Scanner(x));
     ...
     * line.split(",")) .map(fields -> new BusStop(fields[0], fields[1]))
     * .collect(Collectors.toSet()); sc.close();
     */
  ...
  public CompletableFuture<Set<BusStop>> findStopsWith(String name) {
    return getBusStops().thenApply(x -> x.stream()
  ...
  /**
   * Return true if this bus service is equals to another bus service. Two bus
   * services are equal if they have the same id.
   * 
